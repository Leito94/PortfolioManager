...
class MainWindow(qtw.QWidget):
    ...
    def __init__(self):
        ...
        self.mainLayout.layout().addWidget(self.lb_4, 3, 0, 1, 4)
    ...
    def diversify_portfolio(self):
        ...
        while (len(portfolio_df) < 15) & (index < index_num):
            try:
                if momentum_df.loc[index]["names"] not in portfolio_df.names.to_list() and c_list.count(
            ...
            try:
                if value_df.loc[index]["names"] not in portfolio_df.names.to_list() and c_list.count(
            ...
            try:
                if profit_df.loc[index]["names"] not in portfolio_df.names.to_list() and c_list.count(
        ...
        # TODO add the table to the GUI
        # self.createTable(portfolio_df)

        return portfolio_df

    def past_performance(self):
        self.lb_5.setText("plotting past performance...")

        self.progress_bar.setValue(0)
        self.progress_bar.setMaximum(len(portfolio_df) * 4)
        ...
        plt.plot(spy.index, spy.kum_ret, "blue", label="S&P 500 Index")
    ...
    def create_table(self, portfolio_df):
        ...
        self.mainLayout.layout().addWidget(self.tableWidget, 8, 0, 1, 6)
